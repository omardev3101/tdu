...
// Forçamos a URL do Render como fallback seguro
...
export default function Members() {
  ...
  async function loadMembers() {
    try {
      setLoading(true);
      // Usando a rota administrativa correta
  ...
  // Função auxiliar para garantir que a imagem aponte para o lugar certo
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Gestão Interna TDU 7 Caveiras</p>
        </div>
      ...
      <div className="bg-slate-900 rounded-[32px] border border-slate-800 overflow-hidden shadow-2xl">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {members.map(member => (
                <tr key={member.id} className="hover:bg-slate-800/40 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-xs text-slate-400 font-black uppercase tracking-widest">{member.category}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase border ${member.status === 'Ativo' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                      {member.status || 'Pendente'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setSelectedForCard(member)} className="p-2 text-emerald-400 hover:bg-emerald-400/10 rounded-xl" title="Carteirinha"><Scroll size={18} /></button>
                      <button onClick={() => handleEditMember(member)} className="p-2 text-blue-400 hover:bg-blue-400/10 rounded-xl" title="Editar"><Pencil size={18} /></button>
                      <button onClick={() => handleDelete(member.id)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-xl" title="Excluir"><Trash2 size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {loading && (
          <div className="p-20 text-center flex flex-col items-center gap-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Sincronizando Corrente...</span>
          </div>
        )}
      </div>
