...
export default function MemberValidate() {
  ...
  useEffect(() => {
    // Evita chamar a API se o ID for undefined
    if (!id || id === 'undefined') {
        setLoading(false);
        return;
    }
  ...
  // Limpa a URL para evitar barra dupla
  const cleanBaseUrl = API_URL.replace(/\/$/, '');

  return (
    <div className="min-h-screen bg-slate-950 p-4 md:p-10 font-sans flex items-center justify-center">
      <div className="max-w-md w-full bg-slate-900 rounded-[40px] overflow-hidden shadow-2xl border border-slate-800">
        ...
        <div className="p-8 flex flex-col items-center">
          ...
          {/* Foto do Membro - Aumentada para 32 (128px) */}
          <div className="w-32 h-32 rounded-3xl bg-slate-800 border-4 border-slate-800 overflow-hidden shadow-2xl mb-6 flex items-center justify-center relative -mt-16 z-20">
            {member.photo_url ? (
              <img 
                src={member.photo_url.startsWith('http') ? member.photo_url : `${cleanBaseUrl}/uploads/${member.photo_url}`} 
                className="w-full h-full object-cover z-10"
                alt={member.full_name}
                onError={(e) => { e.target.style.display = 'none'; }} 
              />
            ) : null}
            <span className="absolute text-slate-500 font-black uppercase text-4xl">
          ...
          <span className="px-5 py-1.5 bg-red-600 text-white rounded-full text-xs font-black uppercase tracking-widest shadow-lg shadow-red-900/20 mb-8">
            {member.category}
          </span>
